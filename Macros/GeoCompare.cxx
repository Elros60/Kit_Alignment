...
std::map<int, o2::math_utils::Transform3D> transformA;
std::map<int, o2::math_utils::Transform3D> transformB;
...
void GeoCompare(std::string GeoA, std::string GeoB){


    base::GeometryManager::loadGeometry(GeoA.c_str());
    transformation = o2::mch::geo::transformationFromTGeoManager(*gGeoManager);
    for (int i = 0; i < 156; i++) { 
      int iDEN = GetDetElemId(i);
      transformA[iDEN] = transformation(iDEN);
    }
    
    base::GeometryManager::loadGeometry(GeoB.c_str());
    transformation = o2::mch::geo::transformationFromTGeoManager(*gGeoManager);
    for (int i = 0; i < 156; i++) { 
      int iDEN = GetDetElemId(i);
      transformB[iDEN] = transformation(iDEN);
    }
    

    for (int i = 0; i < 156; i++) { 
      int iDEN = GetDetElemId(i);
      auto transform3D_A = transformA[iDEN];
      auto transform3D_B = transformB[iDEN];


      TMatrixD MTransA(3,4);
      TMatrixD MTransB(3,4);

      transform3D_A.GetTransformMatrix(MTransA);
      transform3D_B.GetTransformMatrix(MTransB);
      cout << "===================================================" <<endl;
      cout << "DET ID: " << iDEN <<endl;
      MTransA.Print();
      MTransB.Print();
      cout << "---------------------------------------------------" <<endl;
      (MTransA-MTransB).Print();
      cout << "===================================================" <<endl;
      cout << endl;
      cout << endl;
